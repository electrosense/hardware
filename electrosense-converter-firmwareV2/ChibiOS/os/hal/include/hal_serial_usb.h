...
typedef struct {
  ...
  usbep_t                   int_in;
  /*
   * The SoF interrupt every 1ms generates quite a lot of EMI. This callback
   * allows disabling it when not needed
   */
  void(*sofCallback)(USBDriver* driver, bool needSoF);
} SerialUSBConfig;
