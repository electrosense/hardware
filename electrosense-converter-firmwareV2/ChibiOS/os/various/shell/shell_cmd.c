...
static void cmd_threads(void* user, BaseSequentialStream *chp, int argc, char *argv[]) {
...
#if (SHELL_CMD_TEST_ENABLED == TRUE) || defined(__DOXYGEN__)
static void cmd_test(void* user, BaseSequentialStream *chp, int argc, char *argv[]) {
  thread_t *tp;
  (void)argv;
  (void)user;
  if (argc > 0) {
